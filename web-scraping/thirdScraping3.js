...
(async () => {
  ...
  const resourceData = fs
    .readFileSync("./txt_files/resourceTypes3.txt", "utf-8")
    .split("\n");
  const awsProduct = [];
  ...
  fs.writeFile("awsProduct3.json", JSON.stringify(awsProduct), (err) => {
    if (err) {
      console.error("awsProduct3.json 저장 실패");
      console.error(err);
    } else {
      console.log("awsProduct3.json 저장 성공");
    }
  });
