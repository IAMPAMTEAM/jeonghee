...
(async () => {
  ...
  const resourceData = fs
    .readFileSync("./txt_files/resourceTypes4.txt", "utf-8")
    .split("\n");
  const awsProduct = [];
  ...
  fs.writeFile("awsProduct4.json", JSON.stringify(awsProduct), (err) => {
    if (err) {
      console.error("awsProduct4.json 저장 실패");
      console.error(err);
    } else {
      console.log("awsProduct4.json 저장 성공");
    }
  });
