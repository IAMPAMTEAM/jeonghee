...
(async () => {
  ...
  const resourceData = fs
    .readFileSync("./txt_files/resourceTypes1.txt", "utf-8")
    .split("\n");
  const awsProduct = [];
  ...
  fs.writeFile("awsProduct1.json", JSON.stringify(awsProduct), (err) => {
    if (err) {
      console.error("awsProduct1.json 저장 실패");
      console.error(err);
    } else {
      console.log("awsProduct1.json 저장 성공");
    }
  });
