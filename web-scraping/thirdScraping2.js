...
(async () => {
  ...
  const resourceData = fs
    .readFileSync("./txt_files/resourceTypes2.txt", "utf-8")
    .split("\n");
  const awsProduct = [];
  ...
  fs.writeFile("awsProduct2.json", JSON.stringify(awsProduct), (err) => {
    if (err) {
      console.error("awsProduct2.json 저장 실패");
      console.error(err);
    } else {
      console.log("awsProduct2.json 저장 성공");
    }
  });
